...
- **NotificationMonitor** — Hybrid detection: AXObserver watches `com.apple.notificationcenterui` for real-time triggers (staggered burst reads on each AX event to beat commit/dismiss races), plus a 2s fallback poll timer. Reads the system notification SQLite DB (`~/Library/Group Containers/group.com.apple.usernoted/db2/db`, read-only WAL mode). Parses binary plist blobs from the `record` table (`req.titl`, `req.body`). If a burst finds no row while the DB is open, History may record a `speedy_dismiss` placeholder.
